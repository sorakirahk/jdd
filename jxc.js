...
let jdNotify = true;
let superMarketUpgrade = true;//
let businessCircleJump = true;//
let drawLotteryFlag = true
...
async function jdSuperMarket() {
  await receiveGoldCoin();
  if ($.goldCoinData.data.bizCode === 300) {
    $.msg($.name, `【提示】京东账号${$.index}${UserName} cookie已过期！请先获取cookie`, 'https://bean.m.jd.com/', {"open-url": "https://bean.m.jd.com/"});
    if ($.index === 1) {
      $.setdata('', 'CookieJD');
    } else if ($.index === 2){
      $.setdata('', 'CookieJD2');
